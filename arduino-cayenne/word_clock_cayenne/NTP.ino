...
void updateTimeFromNTP() {
  ...
  if (!cb) {
    ...
    int tz = baseTimezone;                                            // adjust for EST time zone
    DateTime gt(epoch - (tz*60*60));                       // obtain date & time based on NTP-derived epoch...
    tz = IsDST(gt.month(), gt.day(), gt.dayOfTheWeek()) ? baseTimezone -1 : baseTimezone;
    DateTime ntime(epoch - (tz*60*60));                    
